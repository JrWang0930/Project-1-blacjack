...
public class Blackjack implements BlackjackEngine {
	...
	public Blackjack(Random randomGenerator, int numberOfDecks) {
		...
		//Creat two arraylists to store siut and value
	...
	public void createAndShuffleGameDeck() {
		...
		for (int i = 0; i < numberOfDecks; i++) {
			...
			}//Creat two arraylist storing suit and value before, and iterate to create card
		}
		Collections.shuffle(deck, rg);//Shuffle Game Deck
	}
	...
	public void deal() {
		...
		Card c = this.deck.remove(0);//Get the first Card on the Deck
		c.setFaceUp();//Set face up
		this.playersCards.add(c);//Give that card to player
		c = this.deck.remove(0);//Get the second Card on the Deck
		c.setFaceDown();//Set face down
		this.dealersCards.add(c);//Give that card to dealer
